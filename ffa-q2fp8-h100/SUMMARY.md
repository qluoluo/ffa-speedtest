# Q2FP8 Decode Acceleration (ffa-q2fp8-h100)

## Overview
This directory implements a custom decode-time attention path that stores K in 2-bit quantized form with an FP8 residual and prunes blocks using a threshold heuristic. It also includes a fused K-projection + RoPE + quantization path and benchmarking utilities. The H100 variant adds kernel autotuning, a blocked stage-2 reduction, and a reusable workspace to reduce per-step overhead.
...
- Stage 1 kernel (`attn_forward_stage1_fused_threshold_qbits`):
  ...
  - Stores per-block partial outputs (m, l, o) and a keep mask (writes 0/1 for all blocks).
- Stage 2 kernel (`attn_forward_stage2_masked`):
  - Merges kept blocks with log-sum-exp accumulation to produce the final output.
  - Uses a blocked reduction to reduce serial work per head.
- Python wrapper `attn_forward_decode_quantized` validates shapes/dtypes and exposes:
  - `BS` (block size), `SBS` (sub-block size), `delta` (threshold margin),
    `precomputed_threshold` (optional), `use_fp8_residual`, and `workspace` (optional).
