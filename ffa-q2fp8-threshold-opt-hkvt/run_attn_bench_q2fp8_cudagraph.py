...
def main():
    ...
    def build_quant_inputs(length: int):
        ...
        # Kernel layout: [B, HKV, T, ...] for K/V caches.
        k_q_kernel = k_q.permute(0, 2, 1, 3).contiguous()
        k_residual_kernel = k_residual.permute(0, 2, 1, 3).contiguous()
        v_kernel = v.permute(0, 2, 1, 3).contiguous()
        quant_inputs = dict(
            q=q_1,
            k_q=k_q_kernel,
            k_scale=k_scale,
            v=v_kernel,
            k_residual=k_residual_kernel,
        )
