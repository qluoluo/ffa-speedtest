# CUDAGraph wrapper for Q2FP8 decode kernel (symmetric quantization, aggressive low-reg).
...
def attn_compute_threshold_qbits(
    ...
    q_zero_sum = tl.sum(
        (tl.load(q_ptrs0, mask=row_mask[:, None] & mask_k0[None, :], other=0.0).to(tl.float16)
         * scale0[None, :].to(tl.float16)).to(tl.float32),
        axis=1,
    )
    q_zero_sum += tl.sum(
        (tl.load(q_ptrs1, mask=row_mask[:, None] & mask_k1[None, :], other=0.0).to(tl.float16)
         * scale1[None, :].to(tl.float16)).to(tl.float32),
        axis=1,
    )
    q_zero_sum += tl.sum(
        (tl.load(q_ptrs2, mask=row_mask[:, None] & mask_k2[None, :], other=0.0).to(tl.float16)
         * scale2[None, :].to(tl.float16)).to(tl.float32),
        axis=1,
    )
    q_zero_sum += tl.sum(
        (tl.load(q_ptrs3, mask=row_mask[:, None] & mask_k3[None, :], other=0.0).to(tl.float16)
         * scale3[None, :].to(tl.float16)).to(tl.float32),
        axis=1,
    )
    q_zero_sum *= -QZERO
    ...
    q_scaled = tl.load(q_ptrs0, mask=row_mask[:, None] & mask_k0[None, :], other=0.0).to(tl.float16)
    q_scaled = q_scaled * scale0[None, :].to(tl.float16)
    b_s0 = tl.dot(q_scaled, ((kq_packed0 >> 0) & QMAX).to(tl.float16), out_dtype=tl.float32)
    q_scaled = tl.load(q_ptrs1, mask=row_mask[:, None] & mask_k1[None, :], other=0.0).to(tl.float16)
    q_scaled = q_scaled * scale1[None, :].to(tl.float16)
    b_s0 += tl.dot(q_scaled, ((kq_packed0 >> 2) & QMAX).to(tl.float16), out_dtype=tl.float32)
    q_scaled = tl.load(q_ptrs2, mask=row_mask[:, None] & mask_k2[None, :], other=0.0).to(tl.float16)
    q_scaled = q_scaled * scale2[None, :].to(tl.float16)
    b_s0 += tl.dot(q_scaled, ((kq_packed0 >> 4) & QMAX).to(tl.float16), out_dtype=tl.float32)
    q_scaled = tl.load(q_ptrs3, mask=row_mask[:, None] & mask_k3[None, :], other=0.0).to(tl.float16)
    q_scaled = q_scaled * scale3[None, :].to(tl.float16)
    b_s0 += tl.dot(q_scaled, ((kq_packed0 >> 6) & QMAX).to(tl.float16), out_dtype=tl.float32)
    ...
    q_scaled = tl.load(q_ptrs0, mask=row_mask[:, None] & mask_k0[None, :], other=0.0).to(tl.float16)
    q_scaled = q_scaled * scale0[None, :].to(tl.float16)
    b_s1 = tl.dot(q_scaled, ((kq_packed1 >> 0) & QMAX).to(tl.float16), out_dtype=tl.float32)
    q_scaled = tl.load(q_ptrs1, mask=row_mask[:, None] & mask_k1[None, :], other=0.0).to(tl.float16)
    q_scaled = q_scaled * scale1[None, :].to(tl.float16)
    b_s1 += tl.dot(q_scaled, ((kq_packed1 >> 2) & QMAX).to(tl.float16), out_dtype=tl.float32)
    q_scaled = tl.load(q_ptrs2, mask=row_mask[:, None] & mask_k2[None, :], other=0.0).to(tl.float16)
    q_scaled = q_scaled * scale2[None, :].to(tl.float16)
    b_s1 += tl.dot(q_scaled, ((kq_packed1 >> 4) & QMAX).to(tl.float16), out_dtype=tl.float32)
    q_scaled = tl.load(q_ptrs3, mask=row_mask[:, None] & mask_k3[None, :], other=0.0).to(tl.float16)
    q_scaled = q_scaled * scale3[None, :].to(tl.float16)
    b_s1 += tl.dot(q_scaled, ((kq_packed1 >> 6) & QMAX).to(tl.float16), out_dtype=tl.float32)
...
def attn_forward_stage1_fused_threshold_qbits(
    ...
    for sb in tl.static_range(NSB):
        ...
        # Re-load q per sub-block; consume one slice at a time to reduce registers.
        q_scaled = tl.load(
            q_ptrs0,
            mask=row_mask[:, None] & mask_k0[None, :],
            other=0.0,
        ).to(tl.float16)
        q_scaled = q_scaled * scale0[None, :].to(tl.float16)
        b_s_q = tl.dot(q_scaled, ((kq_packedsb >> 0) & QMAX).to(tl.float16), out_dtype=tl.float32)
        q_scaled = tl.load(
            q_ptrs1,
            mask=row_mask[:, None] & mask_k1[None, :],
            other=0.0,
        ).to(tl.float16)
        q_scaled = q_scaled * scale1[None, :].to(tl.float16)
        b_s_q += tl.dot(q_scaled, ((kq_packedsb >> 2) & QMAX).to(tl.float16), out_dtype=tl.float32)
        q_scaled = tl.load(
            q_ptrs2,
            mask=row_mask[:, None] & mask_k2[None, :],
            other=0.0,
        ).to(tl.float16)
        q_scaled = q_scaled * scale2[None, :].to(tl.float16)
        b_s_q += tl.dot(q_scaled, ((kq_packedsb >> 4) & QMAX).to(tl.float16), out_dtype=tl.float32)
        q_scaled = tl.load(
            q_ptrs3,
            mask=row_mask[:, None] & mask_k3[None, :],
            other=0.0,
        ).to(tl.float16)
        q_scaled = q_scaled * scale3[None, :].to(tl.float16)
        b_s_q += tl.dot(q_scaled, ((kq_packedsb >> 6) & QMAX).to(tl.float16), out_dtype=tl.float32)
        ...
        if not prune_blk:
            if USE_FP8_RESIDUAL:
                ...
                # Stream residual dequant to reduce register pressure.
                inv_scale0 = (1.0 / scale0).to(tl.float16)
                inv_scale1 = (1.0 / scale1).to(tl.float16)
                inv_scale2 = (1.0 / scale2).to(tl.float16)
                inv_scale3 = (1.0 / scale3).to(tl.float16)
                ...
                k_res0 = k_res0 * inv_scale0[:, None]
                b_s_res = tl.dot(q_scaled, k_res0, out_dtype=tl.float32)
                ...
                k_res1 = k_res1 * inv_scale1[:, None]
                b_s_res += tl.dot(q_scaled, k_res1, out_dtype=tl.float32)
                ...
                k_res2 = k_res2 * inv_scale2[:, None]
                b_s_res += tl.dot(q_scaled, k_res2, out_dtype=tl.float32)
                ...
                k_res3 = k_res3 * inv_scale3[:, None]
                b_s_res += tl.dot(q_scaled, k_res3, out_dtype=tl.float32)
