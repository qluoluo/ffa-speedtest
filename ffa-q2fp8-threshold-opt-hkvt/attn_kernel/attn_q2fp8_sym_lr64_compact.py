...
def attn_compute_threshold_qbits(
    ...
    t_mask0 = offs_t0 < T
    base_tok0_q = pid_b * (HKV * T * K_PACKED) + pid_hkv * (T * K_PACKED) + offs_t0 * K_PACKED
    tl.multiple_of(base_tok0_q, K_PACKED)

    tb1 = NTB - 1
    offs_t1 = tb1 * T_BS + tl.arange(0, T_BS)
    t_mask1 = offs_t1 < T
    base_tok1_q = pid_b * (HKV * T * K_PACKED) + pid_hkv * (T * K_PACKED) + offs_t1 * K_PACKED
...
def attn_forward_stage1_fused_threshold_qbits_compact(
    ...
    for sb in tl.static_range(NSB):
        ...
        base_toksb_q = pid_b * (HKV * T * K_PACKED) + pid_hkv * (T * K_PACKED) + offs_t_sb * K_PACKED
        base_toksb_k = pid_b * (HKV * T * K) + pid_hkv * (T * K) + offs_t_sb * K
        ...
        if not prune_blk:
            ...
            if need_v:
                v_ptrs = v + pid_b * (HKV * T * V) + pid_hkv * (T * V) + offs_t_sb[:, None] * V + v_offs[None, :]
...
def attn_forward_decode_quantized(
    ...
    k_q: torch.Tensor,         # [B, HKV, T, ceil(K / (8 / k_bits))], packed quantized ints
    k_scale: torch.Tensor,     # [B, HKV, K] (token dimension removed)
    v: torch.Tensor,           # [B, HKV, T, V]
    k_residual: torch.Tensor | None = None,  # [B, HKV, T, K], fp8 residual
    k_bits: int = 2,
    scale: float = None,
    BS: int = 128,
    SBS: int | None = None,
    ...
    **kwargs,
    ...
    B, Tq, HQ, K = q.shape
    Bk, HKV, T, K_packed = k_q.shape
    Bv, HKVv, Tv, V = v.shape
    if 8 % k_bits != 0:
    ...
    if k_residual is not None:
        Bk_r, HKV_r, T_r, K_r = k_residual.shape
        assert (
            B == Bk == Bv == Bk_r
            and Tq == 1
            and Tv == T == T_r
            and HKVv == HKV == HKV_r
            and K == K_r
        ), "K/V layouts must be [B, HKV, T, D]"
    else:
        assert B == Bk == Bv and Tq == 1 and Tv == T and HKVv == HKV, "K/V layouts must be [B, HKV, T, D]"
    G = HQ // HKV
