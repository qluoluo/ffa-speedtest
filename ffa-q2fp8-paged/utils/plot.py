...
def plot_speed_curve(
    ...
    line_fused, = ax1.plot(x_lengths, fused_ms_list, label="Triton fused (paged)", marker="o", markersize=2, color="tab:blue")
    ...
    ax1.set_title(
        f"Layer {layer_idx} Paged Attention Speed vs Length (Tmax={Tmax_k_str}, BS={BS}, SBS={SBS}, delta={delta}{kernel_info})"
    )

    ax1.grid(True, linestyle="--", alpha=0.4)

    lines = [line_fused, line_flash]
    labels = ["Triton fused (paged)", "FlashAttn"]
    ...
    if attn_kernel_name:
        plot_path = out_dir / f"layer_{layer_idx}_speed_Tmax{Tmax_k_str}_{attn_kernel_name}_paged.png"
    else:
        plot_path = out_dir / f"layer_{layer_idx}_speed_Tmax{Tmax_k_str}_paged.png"
