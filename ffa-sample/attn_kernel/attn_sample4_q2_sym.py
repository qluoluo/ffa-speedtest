# Sample4 + 2-bit 对称量化加速方案
# - 每个 Block128 存储 4 个均匀采样点的 2-bit 量化 K 值用于快速筛选
# - 筛选阶段只用 4 个采样点计算近似分数（计算量 4/128 = 1/32）
# - 存储量化 K: 2-bit 对称量化（存储量 2/16 = 1/8）
# - 保留完整 K/V cache 用于非剪枝 block 的精确计算
#
# 数据布局：
# - k_sample_q: [B, num_blocks, HKV, 4, K_packed] 或 [B, num_blocks, HKV, K_packed_4]
# - k_sample_scale: [B, num_blocks, HKV, K]
...
@triton.jit
def attn_compute_threshold_sample4(
    q, k_sample_q, k_sample_scale,
    th_out,
    scale, T, NTB, delta,
    ...
    k_sample_q: [B, num_blocks, HKV, NUM_SAMPLES, K_packed]
    k_sample_scale: [B, num_blocks, HKV, K]
    """
    ...
    for k_start in tl.static_range(0, K, BK):
        ...
        # 加载 scale for block 0
        scale_ptrs0 = k_sample_scale + pid_b * (NTB * HKV * K) + tb0 * (HKV * K) + pid_hkv * K + offs_k
        scale_sub0 = tl.load(scale_ptrs0, mask=k_mask, other=0.0).to(tl.float32)
        q_scaled_sub0 = q_sub * scale_sub0[None, :].to(tl.float16)

        # 加载 scale for block NTB-1
        scale_ptrs1 = k_sample_scale + pid_b * (NTB * HKV * K) + tb1 * (HKV * K) + pid_hkv * K + offs_k
        scale_sub1 = tl.load(scale_ptrs1, mask=k_mask, other=0.0).to(tl.float32)
        q_scaled_sub1 = q_sub * scale_sub1[None, :].to(tl.float16)

        # 累加 q_zero_sum (使用 block 0 的 scale)
        q_zero_sum += tl.sum(q_scaled_sub0.to(tl.float32), axis=1)

        # 加载采样点的量化 K: [K_packed, NUM_SAMPLES]
        # k_sample_q: [B, num_blocks, HKV, NUM_SAMPLES, K_packed]
        kq_base0 = k_sample_q + pid_b * (NTB * HKV * NUM_SAMPLES * K_PACKED) + \
                   tb0 * (HKV * NUM_SAMPLES * K_PACKED) + pid_hkv * (NUM_SAMPLES * K_PACKED)
        kq_base1 = k_sample_q + pid_b * (NTB * HKV * NUM_SAMPLES * K_PACKED) + \
                   tb1 * (HKV * NUM_SAMPLES * K_PACKED) + pid_hkv * (NUM_SAMPLES * K_PACKED)

        # 加载 [K_packed, NUM_SAMPLES] 的数据
        kq_ptrs0 = kq_base0 + sample_offs[None, :] * K_PACKED + pack_idx[:, None]
        kq_ptrs1 = kq_base1 + sample_offs[None, :] * K_PACKED + pack_idx[:, None]

        kq_tile0 = tl.load(kq_ptrs0, mask=k_mask[:, None], other=0).to(tl.int32)
        kq_tile0 = ((kq_tile0 >> pack_shifts[:, None]) & QMAX).to(tl.float16)

        kq_tile1 = tl.load(kq_ptrs1, mask=k_mask[:, None], other=0).to(tl.int32)
        kq_tile1 = ((kq_tile1 >> pack_shifts[:, None]) & QMAX).to(tl.float16)

        # 点积: [BM_DOT, K] x [K, NUM_SAMPLES] -> [BM_DOT, NUM_SAMPLES]
...
@triton.jit
def attn_forward_stage1_sample4(
    q, k_sample_q, k_sample_scale, k_full, v,
    m_buf, l_buf, o_buf,
    ...
    """
    Stage1: 使用 4 个采样点快速筛选，非剪枝 block 使用完整 K 精确计算。

    k_sample_q: [B, num_blocks, HKV, NUM_SAMPLES, K_packed] - 采样点的量化 K
    k_sample_scale: [B, num_blocks, HKV, K] - 每个 block 的 scale
    k_full: [B, T, HKV, K] - 完整 K cache
    """
    ...
    if USE_EXT_TH:
        ...
        for k_start in tl.static_range(0, K, BK):
            ...
            q_zero_sum_th += tl.sum(q_scaled_sub0.to(tl.float32), axis=1)
    ...
    for sb in tl.static_range(NSB):
        ...
        q_zero_sum_sb = tl.zeros([BM_DOT], tl.float32)
        ...
        for k_start in tl.static_range(0, K, BK):
            ...
            scale_ptrs_sb = k_sample_scale + pid_b * (NTB * HKV * K) + pid_tb * (HKV * K) + pid_hkv * K + offs_k
            scale_sub_sb = tl.load(scale_ptrs_sb, mask=k_mask, other=0.0).to(tl.float32)
            q_scaled_sub_sb = q_sub * scale_sub_sb[None, :].to(tl.float16)

            q_zero_sum_sb += tl.sum(q_scaled_sub_sb.to(tl.float32), axis=1)

            kq_base_sb = k_sample_q + pid_b * (NTB * HKV * NUM_SAMPLES * K_PACKED) + \
                         pid_tb * (HKV * NUM_SAMPLES * K_PACKED) + pid_hkv * (NUM_SAMPLES * K_PACKED)

            kq_ptrs_sb = kq_base_sb + sample_offs[None, :] * K_PACKED + pack_idx[:, None]
            kq_tile_sb = tl.load(kq_ptrs_sb, mask=k_mask[:, None], other=0).to(tl.int32)
            kq_tile_sb = ((kq_tile_sb >> pack_shifts[:, None]) & QMAX).to(tl.float16)

            b_s_sample += tl.dot(q_scaled_sub_sb, kq_tile_sb, out_dtype=tl.float32)

        q_zero_sum_sb *= -QZERO
        b_s_sample = (b_s_sample + q_zero_sum_sb[:, None]) * scale * RCP_LN2
...
def quantize_k_sample4_2bit_symmetric(
    ...
    """
    对 K 进行 block 级别的 Sample4 + 2-bit 对称量化。
    ...
    Returns:
        k_sample_q: [B, num_blocks, HKV, NUM_SAMPLES, K_packed] 量化的采样 K
        k_sample_scale: [B, num_blocks, HKV, K] 每个 block 的 scale
    """
    ...
    k_samples = k_samples.permute(0, 1, 3, 2, 4).contiguous()

    # 2-bit 对称量化
    # scale = max(|k|) / 1.5 (per block, per head, per dim)
    k_absmax = k_samples.abs().amax(dim=3, keepdim=True)  # [B, num_blocks, HKV, 1, K]
    scale = (k_absmax / 1.5).clamp_min(1e-6)  # [B, num_blocks, HKV, 1, K]

    # 量化: k_q = round(k / scale + 1.5), clamp to [0, 3]
    k_q = torch.round(k_samples / scale + 1.5).clamp(0, 3).to(torch.uint8)

    # 打包 4 个 2-bit 值到一个 byte
    k_bits = 2
    vals_per_byte = 8 // k_bits
    K_packed = (K + vals_per_byte - 1) // vals_per_byte
    pad_K = K_packed * vals_per_byte - K
    if pad_K > 0:
        k_q = torch.nn.functional.pad(k_q, (0, pad_K), value=0)

    k_q = k_q.view(B, num_blocks, HKV, num_samples, K_packed, vals_per_byte)
    k_q_packed = (
        k_q[..., 0]
        | (k_q[..., 1] << 2)
        | (k_q[..., 2] << 4)
    ...
    # scale 去掉 num_samples 维度
    k_sample_scale = scale.squeeze(3).contiguous()  # [B, num_blocks, HKV, K]

    return k_q_packed, k_sample_scale


def attn_forward_decode_sample4(
    q: torch.Tensor,              # [B, 1, HQ, K]
    k_sample_q: torch.Tensor,     # [B, num_blocks, HKV, NUM_SAMPLES, K_packed]
    k_sample_scale: torch.Tensor, # [B, num_blocks, HKV, K]
    ...
    **kwargs,
    ...
    """
    Sample4 + 2-bit 量化加速的 decode attention。

    使用 4 个采样点的量化 K 快速筛选 block，非剪枝 block 使用完整 K 精确计算。
    """
    assert q.is_cuda and k_sample_q.is_cuda and k_full.is_cuda and v.is_cuda
    if k_bits != 2:
        raise ValueError(f"Currently only supports 2-bit quantization, got k_bits={k_bits}")

    B, Tq, HQ, K = q.shape
    Bk, num_blocks, HKV, num_samples, K_packed = k_sample_q.shape
    ...
    assert q.is_contiguous() and k_sample_q.is_contiguous() and k_full.is_contiguous() and v.is_contiguous()
    assert k_sample_scale.is_contiguous()
    ...
    if precomputed_threshold is not None:
        ...
        def _launch_threshold():
            attn_compute_threshold_sample4[(B, HKV)](
                q, k_sample_q, k_sample_scale,
                threshold_buf,
                scale, T, NTB, delta,
                B=B, HKV=HKV, HQ=HQ, K=K, K_PACKED=K_packed, G=G, BS=BS,
                K_BITS=k_bits, NUM_SAMPLES=num_samples,
                **th_kwargs,
            )
    ...
    def _launch_stage1():
        attn_forward_stage1_sample4[(NTB, B, HKV)](
            q, k_sample_q, k_sample_scale, k_full, v,
            m_buf, l_buf, o_buf,
            ...
            K_BITS=k_bits, USE_EXT_TH=use_ext_th, NUM_SAMPLES=num_samples,
            **s1_kwargs,
        )
...
class CUDAGraphDecodeRunnerSample4Q2:
    """Capture and replay the Sample4+Q2 decode kernel with static buffers."""
