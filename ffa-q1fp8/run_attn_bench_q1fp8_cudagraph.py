# Benchmarking & plotting for Q1FP8 decode with CUDAGraph.
import argparse
import importlib
import inspect
import json
...
def parse_args():
    p = argparse.ArgumentParser(description="Benchmark Q1FP8 decode with CUDAGraph.")
    ...
    p.add_argument(
        "--attn-kernel",
        type=str,
        default="attn_q1fp8_sym_mask",
        help="Kernel module name under attn_kernel/ (e.g. attn_q1fp8_sym_mask).",
    )
    p.add_argument(
        "--max-length",
        type=int,
    ...
    p.add_argument(
        "--with-q1",
        action="store_true",
        help="Also benchmark the non-CUDAGraph Q1FP8 baseline.",
    )
...
def quantize_k_1bit_fp8_residual(
    k: torch.Tensor,
    ...
    if k_bits != 1:
        raise ValueError(f"Expected 1-bit quantization, got k_bits={k_bits}")
    qmax = (1 << k_bits) - 1
    k_min = k.amin(dim=1)
    k_max = k.amax(dim=1)
    scale = ((k_max - k_min).clamp_min(1e-6) / qmax).contiguous()
    zero = k_min.contiguous()
    k_q = torch.round((k - zero[:, None, :, :]) / scale[:, None, :, :]).clamp(0, qmax).to(torch.uint8)
    k_dequant = (
    ...
    # Pack 8x1-bit values into a single byte to avoid storing each 1-bit value as uint8
    B, T, HKV, K = k_q.shape
    values_per_byte = 8  # 8 bits / 1 bit
...
def quantize_k_1bit_fp8_residual_symmetric(
    k: torch.Tensor,
    fp8_dtype: torch.dtype = torch.float8_e5m2,
    k_bits: int = 1,
):
...
def _resolve_cudagraph_runner(cg_module):
    for name in (
        "CUDAGraphDecodeRunnerQ1FP8Vec",
        "CUDAGraphDecodeRunnerQ1FP8",
        "CUDAGraphDecodeRunnerQ1FP8ReuseBS",
    ):
...
def save_raw_cache(path, meta: dict, lengths, q1_ms, q1_cg_ms, flash_ms, skip_ratios):
    path = Path(path)
    payload = {
        "meta": meta,
        "lengths": [int(x) for x in lengths],
        "q1_ms": [None if x is None else float(x) for x in q1_ms],
        "q1_cg_ms": [None if x is None else float(x) for x in q1_cg_ms],
...
def plot_curve(
    ...
    lines = []
    labels = []
    if q1_ms_list is not None and any(x is not None for x in q1_ms_list):
        line_q1, = ax1.plot(
            x_lengths,
            q1_ms_list,
            label="Q1FP8",
        ...
        lines.append(line_q1)
        labels.append("Q1FP8")
    lines.append(line_q1_cg)
    labels.append("Q1FP8 CUDAGraph")
...
def main():
    ...
    def build_quant_inputs(length: int):
        ...
        k_q_asym, k_scale_asym, k_zero_asym, k_residual_asym = quantize_k_1bit_fp8_residual(k)
        k_q_sym, k_scale_sym, k_residual_sym = quantize_k_1bit_fp8_residual_symmetric(k)
    ...
    try:
        ...
        if use_cache:
            ...
            cache_has_q1 = bool(cached_q1_ms_list) and all(x is not None for x in cached_q1_ms_list)
            if run_q1 and not cache_has_q1:
                print(f"[Info] Cached results missing Q1 baseline; rerunning to collect it.")
                use_cache = False
        if use_cache:
            if run_q1:
                q1_ms_list = cached_q1_ms_list
            else:
                q1_ms_list = [None] * len(q1_cg_ms_list)
            ...
            if args.profile_kernels:
                ...
                if kernel_profile is not None and "kernel_profile" not in _meta:
                    ...
                    save_raw_cache(cache_path, _meta, x_lengths, q1_ms_list, q1_cg_ms_list, flash_ms_list, skip_ratios)
            ...
            q1_ms_list, q1_cg_ms_list, flash_ms_list, skip_ratios = [], [], [], []
            ...
            for L in tqdm(lengths, desc=f"delta={delta:g}, layers{layer_range_str}(bsz={bsz})"):
                ...
                ms_q1 = None
                if run_q1:
                    ms_q1 = benchmark(run_q1, iters=iters, warmup=warmup)
                ms_q1_cg = benchmark(run_q1_cg, iters=iters, warmup=warmup)
                ms_flash = None
                if flash_attn_compute is not None:
                    ms_flash = benchmark(run_flash, iters=iters, warmup=warmup)

                q1_ms_list.append(ms_q1)
                q1_cg_ms_list.append(ms_q1_cg)
            ...
            ensure_raw_dir()
            save_raw_cache(cache_path, meta, x_lengths, q1_ms_list, q1_cg_ms_list, flash_ms_list, skip_ratios)
        ...
        if flash_ms_list[-1] is not None:
            if q1_ms_list[-1] is not None and q1_ms_list[-1] > 0:
                speedup = flash_ms_list[-1] / q1_ms_list[-1]
                speedup_parts.append(f"Speedup={speedup:.2f}x")
            if q1_cg_ms_list[-1] > 0:
                speedup_cg = flash_ms_list[-1] / q1_cg_ms_list[-1]
        ...
        print(
            ...
            + (f"Q1={q1_ms_list[-1]:.3f} ms, " if q1_ms_list[-1] is not None else "")
            + f"Q1_CG={q1_cg_ms_list[-1]:.3f} ms"
