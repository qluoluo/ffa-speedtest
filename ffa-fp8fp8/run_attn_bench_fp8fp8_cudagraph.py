# Benchmarking & plotting for FP8+FP8 residual decode with CUDAGraph.
import argparse
import json
import math
import re
...
def parse_args():
    p = argparse.ArgumentParser(description="Benchmark FP8+FP8 residual decode with CUDAGraph.")
...
def save_raw_cache(path, meta: dict, lengths, fp8_ms, fp8_cg_ms, flash_ms, skip_ratios):
    path = Path(path)
    payload = {
        "meta": meta,
        "lengths": [int(x) for x in lengths],
        "fp8_ms": [float(x) for x in fp8_ms],
        "fp8_cg_ms": [float(x) for x in fp8_cg_ms],
...
def main():
    ...
    attn_kernel_name = "attn_kernel_v1210_fused_bsz_fp8fp8"
    ...
    try:
        ...
        if flash_ms_list[-1] is not None:
            speedup = flash_ms_list[-1] / fp8_ms_list[-1] if fp8_ms_list[-1] > 0 else float("inf")
            speedup_cg = flash_ms_list[-1] / fp8_cg_ms_list[-1] if fp8_cg_ms_list[-1] > 0 else float("inf")
