...
def attn_forward_stage1_fused_threshold(
    ...
    USE_FP16_RECOMPUTE: tl.constexpr = False,# 非prune块是否用FP16重算
):
    # 3D grid = (NTB, B, HKV):
    ...
    for sb in tl.static_range(NSB):
        ...
        k_tile_hi = tl.load(kb_ptrs, mask=(TRUE_K[:, None] & t_mask_sb[None, :]), other=0.0).to(tl.float16)

        b_s_hi   = tl.dot(q_tile, k_tile_hi, out_dtype=tl.float32) * scale * RCP_LN2
        b_s_act = tl.where(t_mask_sb[None, :], b_s_hi, NEG_INF)
        ...
        if not prune_blk:
            if USE_FP16_RECOMPUTE:
                kfp_ptrs = k_fp16 + pid_b * (T * HKV * K) + (offs_t_sb[None, :] * (HKV * K)) + (pid_hkv * K) + offs_k[:, None]
                k_tile_fp = tl.load(kfp_ptrs, mask=(TRUE_K[:, None] & t_mask_sb[None, :]), other=0.0).to(tl.float16)
...
def attn_forward_decode(
    ...
    recompute_fp16: bool = True,                        # 非prune块是否用FP16重算
):
    ...
    use_fp16_recompute = recompute_fp16 and (k_fp16 is not None)

    # Stage 1：grid 改为 (NTB, B, HKV) 以使时间块维度成为最快变化维
    attn_forward_stage1_fused_threshold[(NTB, B, HKV)](
        q, k_hi8, k_fp16 if k_fp16 is not None else k_hi8, v,
        m_buf, l_buf, o_buf,
        mask_buf,
        scale, T, NTB, NTBS, delta,
        threshold_buf,
        ...
        USE_FP16_RECOMPUTE=use_fp16_recompute,
    )
