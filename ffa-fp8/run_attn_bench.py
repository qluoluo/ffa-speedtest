...
def build_plot_dirs(attn_kernel_name, gpu_tag, BS, SBS, delta, bsz, max_length, base_dir: Path):
    ...
    plot_root_dir = (
        ...
        / f"{attn_kernel_name}"
        / gpu_tag
        / (f"BS{BS}_SBS{SBS}_delta{delta}_bsz{bsz}" + (f"_{lmax_name}" if max_length is not None else ""))
    )
