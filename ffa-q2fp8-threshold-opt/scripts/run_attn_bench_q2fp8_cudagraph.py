...
# Ensure package importability - add parent dir before other imports
...
def quantize_k_2bit_fp8_residual_symmetric_perblock(
    k: torch.Tensor,
    BS: int,
    fp8_dtype: torch.dtype = torch.float8_e5m2,
    k_bits: int = 2,
):
    """Per-block symmetric quantization: k_scale shape [B, NTB, HKV, K]."""
    B, T, HKV, K = k.shape
    qmax = (1 << k_bits) - 1
    qzero = qmax / 2.0
    NTB = (T + BS - 1) // BS
    ...
    # Reshape back: [B, T_padded, HKV, K] -> [B, T, HKV, K]
...
def build_plot_dirs(attn_kernel_name, gpu_tag, BS, SBS, delta, layer_indices, bsz, max_length, base_dir: Path, perblock_scale: bool = False):
    ...
    scale_tag = "_perblock" if perblock_scale else ""
    plot_root_dir = (
        base_dir
        / "plot"
        / f"{attn_kernel_name}_cudagraph"
        / gpu_tag
        / (f"delta{delta}_layers{layer_range}_BS{BS}_SBS{SBS}_bsz{bsz}{scale_tag}" + (f"_{lmax_name}" if max_length is not None else ""))
    )
...
def main():
    ...
    print(f"[Info] Quantization mode: {quant_mode}, Per-block scale: {args.perblock_scale}")
    ...
    def build_quant_inputs(length: int):
        ...
        # Choose between global and per-block scale quantization
        if args.perblock_scale:
            k_q_sym, k_scale_sym, k_residual_sym = quantize_k_2bit_fp8_residual_symmetric_perblock(k, BS)
        else:
