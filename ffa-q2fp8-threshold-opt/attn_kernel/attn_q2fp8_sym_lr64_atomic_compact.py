# 速度优先：对称量化 + 低寄存器 BK=64 + Atomic Compact (无 Scan/Scatter)。
# - 对称量化：仅使用 k_scale（无 zero-point），用 QZERO 抵消量化偏置。
# - K 维度按 BK=64 分块（低寄存器路径）。
# - Stage1 先算阈值，若 prune 则跳过；若保留，使用 atomic_add 获取紧凑位置并直接写入。
# - Stage2 遍历 0..min(n_kept, MAX_KEPT) 合并输出。
# CUDAGraph wrapper for Q2FP8 decode kernel (sym + atomic compact + low-reg BK=64).
...
def attn_forward_stage2_atomic_compact(
    ...
    # Iterate linearly over the compacted buffer
    for i in range(MAX_KEPT):
        mask_i = i < n_kept
        
        # Access compacted data directly at index i
        m_b = tl.load(compact_m_buf + pid_b * (HQ * MAX_KEPT) + pid_hq * MAX_KEPT + i, 
                      mask=mask_i, other=neg_inf)
        l_b = tl.load(compact_l_buf + pid_b * (HQ * MAX_KEPT) + pid_hq * MAX_KEPT + i, 
                      mask=mask_i, other=0.0)
        o_b = tl.load(
            compact_o_buf + pid_b * (HQ * MAX_KEPT * V) + pid_hq * (MAX_KEPT * V) + i * V + v_offs,
            mask=mask_i,
            other=0.0,
        )
