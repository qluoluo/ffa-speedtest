# Compact keep-list variant for Q4FP8 decode (symmetric quantization).
...
def attn_compute_threshold_qbits(
    ...
    kq0_1 = ((kq_packed0 >> 4) & QMAX).to(tl.float16)
    ...
    kq1_1 = ((kq_packed1 >> 4) & QMAX).to(tl.float16)
...
def attn_forward_stage1_fused_threshold_qbits(
    ...
    for sb in tl.static_range(NSB):
        ...
        kqsb1 = ((kq_packedsb >> 4) & QMAX).to(tl.float16)
    ...
    **kwargs,
    ...
    def _launch_stage2():
        attn_forward_stage2_compact[(B, HKV, G)](
            ...
            kept_indices, kept_counts,
            o, NTBS,
            B=B, HKV=HKV, G=G, HQ=HQ, V=V, MAX_KEPT=max_kept,
            **s2_kwargs,
        )
...
class CUDAGraphDecodeRunnerQ4FP8:
    ...
    def __init__(
        ...
        *,
        ...
        ntb = triton.cdiv(k_q.shape[1], BS)
        sbs_val = SBS if SBS is not None else BS
        nsb = triton.cdiv(BS, sbs_val)
        ntbs = ntb * nsb
