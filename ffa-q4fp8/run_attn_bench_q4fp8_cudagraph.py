# Benchmarking & plotting for Q4FP8 decode with CUDAGraph.
import argparse
import importlib
import inspect
import json
...
def parse_args():
    p = argparse.ArgumentParser(description="Benchmark Q4FP8 decode with CUDAGraph.")
    ...
    p.add_argument(
        "--attn-kernel",
        type=str,
        default="attn_q4fp8_sym_mask",
        help="Kernel module name under attn_kernel/ (e.g. attn_q4fp8_sym_mask).",
    )
    p.add_argument(
        "--max-length",
        type=int,
    ...
    p.add_argument(
        "--with-q2",
        action="store_true",
        help="Also benchmark the non-CUDAGraph Q4FP8 baseline.",
    )
...
def _pack_k_bits(k_q: torch.Tensor, k_bits: int) -> torch.Tensor:
    if 8 % k_bits != 0:
    ...
    k_q_packed = k_q[..., 0]
    for idx in range(1, values_per_byte):
        k_q_packed |= (k_q[..., idx] << (idx * k_bits))
    return k_q_packed.contiguous()


def quantize_k_4bit_fp8_residual(
    k: torch.Tensor,
    ...
    k_q_packed = _pack_k_bits(k_q, k_bits)
    return k_q_packed, scale, zero, k_residual


def quantize_k_4bit_fp8_residual_symmetric(
    k: torch.Tensor,
    fp8_dtype: torch.dtype = torch.float8_e5m2,
    k_bits: int = 4,
):
...
def main():
    ...
    def build_quant_inputs(length: int):
        ...
        k_q_asym, k_scale_asym, k_zero_asym, k_residual_asym = quantize_k_4bit_fp8_residual(k)
        k_q_sym, k_scale_sym, k_residual_sym = quantize_k_4bit_fp8_residual_symmetric(k)
    ...
    try:
        ...
        if use_cache:
            ...
            if run_q2 and not cache_has_q2:
                print(f"[Info] Cached results missing Q4 baseline; rerunning to collect it.")
                use_cache = False
        if use_cache:
            if run_q2:
        ...
        print(
            ...
            + (f"Q4={q2_ms_list[-1]:.3f} ms, " if q2_ms_list[-1] is not None else "")
            + f"Q4_CG={q2_cg_ms_list[-1]:.3f} ms"
