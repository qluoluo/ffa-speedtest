...
from nf4fp8_cache import NF4Fp8Cache, NF4Fp8StaticCache, resolve_fp8_dtype
from modeling_llama import LlamaForCausalLM

INPUT_TEXT_PATH = THIS_DIR / "input_text.txt"

def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="End-to-end benchmark for nf4+fp8 cache on a Llama model.")
    ...
    p.add_argument("--compare", action="store_true", help="Compare baseline flash attention vs nf4+fp8 decode.")
    p.add_argument("--ffa-decode", action="store_true", help="Enable nf4+fp8 FFA decode kernel path.")
    p.add_argument(
        "--decode-tokens",
        type=int,
        default=1,
    ...
    p.add_argument("--bs", type=int, default=128, help="Block size for nf4+fp8 decode kernel.")
    p.add_argument("--sbs", type=int, default=None, help="Sub-block size for nf4+fp8 decode kernel.")
    p.add_argument("--delta", type=float, default=5.0, help="Delta threshold for nf4+fp8 decode kernel.")
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Use kernel-level CUDA Graphs for nf4+fp8 decode (static cache).",
    )
...
def main() -> None:
    ...
    if (args.ffa_decode or args.compare) and device.type != "cuda":
        raise RuntimeError("nf4+fp8 FFA decode requires a CUDA device.")
    ...
    if args.decode_cudagraph or args.decode_kernel_cudagraph:
        raise ValueError(
            "NF4FP8 e2e does not support --decode-cudagraph/--decode-kernel-cudagraph."
        )
    ...
    if args.compare:
        ...
        run_variant("NF4FP8", use_ffa_decode=True)
