...
from q2fp8_cache import Q2Fp8Cache, Q2Fp8StaticCache
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if past_key_values is not None and not isinstance(past_key_values, Cache):
            raise TypeError(
                "past_key_values must be a Cache (e.g. Q2Fp8Cache/Q2Fp8StaticCache) in "
        ...
        if past_key_values is not None:
            ...
            if isinstance(past_key_values, (Q2Fp8Cache, Q2Fp8StaticCache)):
                # Q2Fp8Cache expects [B, T, HKV, K] to store quantized keys per token.
        ...
        if q_len > 1 and use_ffa_prefill and self.layer_idx in pattern_layers:
            ...
            if isinstance(past_key_values, (Q2Fp8Cache, Q2Fp8StaticCache)):
                # Pull Q2 keys + scales (+ residuals) from cache for the custom decode kernel.
                cache_layer = cache_layer or past_key_values.layers[self.layer_idx]
                k_q = cache_layer.key_q
                k_scale = cache_layer.key_scale
                k_zero = cache_layer.key_zero
                k_residual = cache_layer.key_residual
                if k_q is None or k_scale is None or k_zero is None:
                    raise RuntimeError("Q2Fp8Cache is missing quantized keys for decode.")
                ...
                if use_full_kv:
                    raise RuntimeError(
                        "Q2Fp8 decode does not support use_full_kv; kernel expects T to match sequence length."
                    )
                v_full = value_states.transpose(1, 2)

                if use_fp8_residual and k_residual is None:
                    raise RuntimeError("Q2Fp8Cache is missing fp8 residual keys for decode.")

                from ffa_fwd_decode_q2fp8 import attn_forward_decode_q2fp8
                ...
                if runner is not None:
                    ...
                    decode_result = attn_forward_decode_q2fp8(
                        q=query_states.transpose(1, 2),
                        k_q=k_q,
                        k_scale=k_scale,
                        k_zero=k_zero,
