# Stage timing for Q2FP8 decode kernels.
import argparse
import importlib
import json
import math
...
def parse_args():
    p = argparse.ArgumentParser(description="Stage timing for Q2FP8 decode kernels.")
    p.add_argument(
        "--kernel",
        type=str,
...
def build_output_dirs(attn_kernel_name, gpu_tag, BS, SBS, delta, layer_indices, bsz, max_length, base_dir: Path):
    ...
    root_dir = (
        base_dir
        / "plot"
        / f"{attn_kernel_name}_stage_timing"
        / gpu_tag
...
def make_cache_file_path(
    ...
    fname = (
        ...
        f"_step{step}_it{iters}_wu{warmup}_bsz{bsz}_stage_timing.json"
    )
...
def save_raw_cache(path, meta: dict, lengths, threshold_ms, stage1_ms, stage2_ms, full_ms, skip_ratios):
    ...
    payload = {
        ...
        "threshold_ms": [float(x) for x in threshold_ms],
        "stage1_ms": [float(x) for x in stage1_ms],
        "stage2_ms": [float(x) for x in stage2_ms],
        "full_ms": [float(x) for x in full_ms],
...
def main():
    ...
    if cache_path.exists() and not args.force:
        ...
        for L in tqdm(lengths, desc=f"delta={delta:g}, layers{layer_range_str}(bsz={bsz})"):
            ...
            # Compile once before timing to avoid including JIT in benchmarks.
            run_threshold()
            run_stage1()
            run_stage2()
            run_full()
            ...
            print(
                f"[Stage] T={to_k_str(L)} | th={ms_threshold:.3f} ms | "
    ...
    last_idx = -1
    print(
        f"[Result] Layers {layer_range_str} | bsz={bsz} | T={to_k_str(x_lengths[last_idx])} | "
        f"BS={BS} SBS={SBS} delta={delta} | "
        f"th={threshold_ms_list[last_idx]:.3f} ms, "
        f"s1={stage1_ms_list[last_idx]:.3f} ms, "
        f"s2={stage2_ms_list[last_idx]:.3f} ms, "
        f"sum={threshold_ms_list[last_idx] + stage1_ms_list[last_idx] + stage2_ms_list[last_idx]:.3f} ms, "
        f"full={full_ms_list[last_idx]:.3f} ms"
    )
