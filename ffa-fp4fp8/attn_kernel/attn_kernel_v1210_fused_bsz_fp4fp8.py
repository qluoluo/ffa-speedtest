...
def attn_forward_stage1_fused_threshold_qbits(
    ...
    if USE_EXT_TH:
        ...
        kq_ptrs0 = k_fp4 + base_tok0_q + pack_idx[:, None]
        kq_tile0 = tl.load(kq_ptrs0, mask=(TRUE_K[:, None] & t_mask0[None, :]), other=0).to(tl.int32)
        kq_tile0 = (kq_tile0 >> pack_shifts[:, None]) & tl.full((), QMAX, tl.int32)
        k_tile0 = _fp4_decode(kq_tile0).to(tl.float16)
        ...
        kq_ptrs1 = k_fp4 + base_tok1_q + pack_idx[:, None]
        kq_tile1 = tl.load(kq_ptrs1, mask=(TRUE_K[:, None] & t_mask1[None, :]), other=0).to(tl.int32)
        kq_tile1 = (kq_tile1 >> pack_shifts[:, None]) & tl.full((), QMAX, tl.int32)
        k_tile1 = _fp4_decode(kq_tile1).to(tl.float16)
    ...
    for sb in tl.static_range(NSB):
        ...
        kq_ptrssb = k_fp4 + base_toksb_q + pack_idx[:, None]
        kq_tilesb = tl.load(kq_ptrssb, mask=(TRUE_K[:, None] & t_mask_sb[None, :]), other=0).to(tl.int32)
        kq_tilesb = (kq_tilesb >> pack_shifts[:, None]) & tl.full((), QMAX, tl.int32)
        k_tile_q = _fp4_decode(kq_tilesb).to(tl.float16)
...
def attn_forward_decode_quantized(
    ...
    k_q: torch.Tensor,         # [B, T, HKV, ceil(K / (8 / k_bits))], packed FP4 nibbles
    k_scale: torch.Tensor,     # [B, HKV, K] (unused placeholder for compatibility)
