...
class SimpleCUDAGraphRunner:
    ...
    def replay(
        ...
        # SimpleCUDAGraph uses max_current=1, so we clamp current_len
        # The kernel will handle the actual current_len through the buffer content
        effective_current_len = min(current_len, 1)
        if current_len > 1:
            # If current_len > 1, we can only use the first token in current buffer
            # This is a limitation of SimpleCUDAGraph for performance
            effective_current_len = 1

        # Copy data
        self._copy_to_static(k_q, k_scale, v, k_residual)
        self.static_q.copy_(q)

        if effective_current_len == 1:
            self.static_k_current[:, 0, :, :].copy_(k_current[:, 0, :, :])
            self.static_v_current[:, 0, :, :].copy_(v_current[:, 0, :, :])

        # Replay
        graph, out = self.graphs[effective_current_len]
        graph.replay()
