        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if is_q2fp8_cache:
            if q_len == 1:
                ...
                # Initialize CUDA Graph runner on first decode (if enabled)
                use_cudagraph = attn_settings.get("use_cudagraph", False)
                if use_cudagraph and self.cudagraph_runner is None and not return_skip:
                    try:
                        from simple_cudagraph import SimpleCUDAGraphRunner
                        print(f"[Layer {self.layer_idx}] Initializing CUDA Graph runner...")

                        max_decode_tokens = attn_settings.get("max_decode_tokens", 4096)

                        self.cudagraph_runner = SimpleCUDAGraphRunner(
                            initial_k_q=k_q,
                            initial_k_scale=k_scale,
                            initial_v=v_quantized,
                            initial_k_residual=k_residual,
                            max_decode_tokens=max_decode_tokens,
                            k_bits=decode_kwargs.get('k_bits', 2),
                            scale=decode_kwargs.get('scale'),
                            BS=decode_kwargs.get('BS', 128),
                            SBS=decode_kwargs.get('SBS'),
                            delta=decode_kwargs.get('delta', 5.0),
                            use_fp8_residual=decode_kwargs.get('use_fp8_residual', True),
                            warmup=2,
                        )
                        print(f"[Layer {self.layer_idx}] CUDA Graph initialized successfully!")
                    except Exception as e:
                        print(f"[Layer {self.layer_idx}] CUDA Graph initialization failed: {e}")
                        print(f"[Layer {self.layer_idx}] Falling back to standard path")
                        self.cudagraph_runner = None
