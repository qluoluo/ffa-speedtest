...
- k_current: [B, max_current, HKV, K] 当前未满 block，固定大小 buffer
- v_current: [B, max_current, HKV, V] 当前未满 block 的 V
- current_len: 当前 buffer 的有效长度
...
class Q2FP8SymLayer(CacheLayerMixin):
    ...
    def _ensure_current_buffers(self, key_states: torch.Tensor, value_states: torch.Tensor) -> None:
        ...
        self.k_current = torch.empty(
            (B, self.max_current, HKV, K), device=key_states.device, dtype=key_states.dtype
        )
    ...
    def _get_full_key(self) -> Optional[torch.Tensor]:
        ...
        if self.k_current is not None and self.current_len > 0:
            parts.append(self.k_current.narrow(self.seq_dim, 0, self.current_len))
        if not parts:
            return None
    ...
    def update(
        ...
        while offset < total_len:
            ...
            take = min(space, total_len - offset)
            k_slice = key_states.narrow(self.seq_dim, offset, take)
            v_slice = value_states.narrow(self.seq_dim, offset, take)
