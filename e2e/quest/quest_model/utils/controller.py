...
class InferenceController:
    ...
    def begin_forward(self, seq_len: int, updateTensor: bool = True):
        ...
        if seq_len > 1:
            ...
            # If there is only one page (short prompt), skip top-k setup and treat as single-page decode.
            if cur_page_nums <= 1:
                self.inference_page_budget = cur_page_nums
                self.kv_indptr_for_approx_decode = torch.tensor([0, 0], dtype=torch.int32, device=self.device)
