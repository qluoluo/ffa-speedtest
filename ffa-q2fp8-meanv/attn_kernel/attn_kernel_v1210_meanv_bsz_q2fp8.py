# attn_kernel_v1210_meanv_bsz_q2fp8.py
# 2-bit quantized K with fp8 residual refinement + mean-V fallback.
import math
...
def attn_forward_stage1_fused_threshold_qbits(
    ...
    for sb in tl.static_range(NSB):
        ...
        has_tokens = tl.sum(t_mask_sb.to(tl.int32), axis=0) > 0

        if prune_blk:
            if has_tokens:
                b_p = tl.where(t_mask_sb[None, :], tl.exp2(b_s_act - m_rows_blk[:, None]), 0.0)
                ...
                tl.store(approx_buf + pid_b * (HKV * NTBS) + pid_hkv * NTBS + tb_sb, tl.full((), 1, tl.int8))
        else:
            if USE_FP8_RESIDUAL:
            ...
            tl.store(mask_buf + pid_b * (HKV * NTBS) + pid_hkv * NTBS + tb_sb, tl.full((), 1, tl.int8))
            tl.store(approx_buf + pid_b * (HKV * NTBS) + pid_hkv * NTBS + tb_sb, tl.full((), 0, tl.int8))


@triton.jit
def attn_forward_stage2_masked(
    m_buf, l_buf, o_buf, mask_buf, approx_buf, v_mean, o, NTBS,
    ...
    # Aggregate mean-V (approx) blocks via scalar stats to fold once at the end.
    p_m = neg_inf
    p_acc = tl.zeros((), tl.float32)
    for tb in range(0, NTBS):
        ...
        if keep:
            ...
            is_approx = tl.load(approx_buf + pid_b * (HKV * NTBS) + pid_hkv * NTBS + tb).to(tl.int1)
            if is_approx:
                new_m = tl.maximum(p_m, m_b)
                r_prev = tl.exp2(p_m - new_m)
                r_blk = tl.exp2(m_b - new_m)
                p_acc = p_acc * r_prev + l_b * r_blk
                p_m = new_m
            else:
                ...
                b_m = new_m
    have_any = (b_acc > 0.0) | (p_acc > 0.0)
    new_m = tl.where(have_any, tl.maximum(b_m, p_m), 0.0)
    r_b = tl.exp2(b_m - new_m)
    r_p = tl.exp2(p_m - new_m)
    v_mean_ptrs = v_mean + pid_b * (HKV * V) + pid_hkv * V + v_offs
    v_mean_tile = tl.load(v_mean_ptrs, mask=v_offs < V, other=0.0).to(tl.float32)
    acc = b_acc * r_b + p_acc * r_p
    o_tile = b_o * r_b + v_mean_tile * (p_acc * r_p)
    out_tile = tl.where(have_any, o_tile / acc, tl.zeros([V], tl.float32))
...
def attn_forward_decode_quantized(
    ...
    **kwargs,
    ...
    approx_buf = torch.zeros((B, HKV, NTBS), device=q.device, dtype=torch.int8)
    ...
    if return_skip_ratio:
        approx = approx_buf.to(torch.int32).sum()
        total = approx_buf.numel()
        skip_ratio = float((approx.float() / float(total)).item())

    v_mean = v.mean(dim=1, dtype=torch.float32).to(v.dtype).contiguous()
    attn_forward_stage2_masked[(B, HKV, G)](
        m_buf, l_buf, o_buf,
        mask_buf, approx_buf, v_mean,
        o, NTBS,
