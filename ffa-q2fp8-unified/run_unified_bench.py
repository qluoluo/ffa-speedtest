#!/usr/bin/env python3
# Benchmarking & plotting for Q2FP8 Unified Kernel vs FlashAttention
import argparse
import json
import math
import re
...
from attn_kernel.attn_q2fp8_unified import attn_forward_decode_quantized
...
def parse_args():
    p = argparse.ArgumentParser(description="Benchmark Q2FP8 Unified Kernel vs FlashAttention.")
    ...
    p.add_argument("--warmup", type=int, default=20, help="Benchmark warmup")
...
def plot_curve(
    ...
    lines = [line_unified]
    labels = [f"Q2FP8 Unified (curr={current_len})"]
    ...
    ax1.set_ylabel("Latency per run (ms)", fontsize=12)
    Tmax_k_str = to_k_str(T_full)
    ax1.set_title(
        f"Q2FP8 Unified Kernel vs FlashAttention\n"
        f"Layer {layer_idx} | Tmax={Tmax_k_str}, BS={BS}, SBS={SBS}, delta={delta}, current={current_len}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    if skip_ratios is not None:
        ...
        ax2.set_ylabel("Skip ratio (%)", fontsize=12)
        ax2.set_ylim(0, 100)
    ...
    plot_path = out_dir / f"layer_{layer_idx}_speed_Tmax{Tmax_k_str}_unified_curr{current_len}.png"
...
def main():
    ...
    try:
        if cache_path.exists():
            x_lengths, unified_ms_list, flash_ms_list, skip_ratios, _meta = load_raw_cache(cache_path)
            print(f"[Info] Loaded cached results from {cache_path}")
        else:
            unified_ms_list, flash_ms_list, skip_ratios = [], [], []
            ...
            for L in tqdm(lengths, desc=f"delta={delta:g}, layers{layer_range_str}(bsz={bsz}), curr={current_len}"):
                ...
                if current_len > 0 and L > current_len:
                ...
                ms_unified = benchmark(run_unified, iters=iters, warmup=warmup)
                ms_flash = None
                if flash_attn_compute is not None:
                    ms_flash = benchmark(run_flash, iters=iters, warmup=warmup)

                unified_ms_list.append(ms_unified)
            ...
            ensure_raw_dir()
            save_raw_cache(cache_path, meta, x_lengths, unified_ms_list, flash_ms_list, skip_ratios)
        ...
        if flash_ms_list[-1] is not None:
            speedup = flash_ms_list[-1] / unified_ms_list[-1] if unified_ms_list[-1] > 0 else float("inf")
        ...
        print(
            ...
            f"Unified={unified_ms_list[-1]:.3f} ms"
